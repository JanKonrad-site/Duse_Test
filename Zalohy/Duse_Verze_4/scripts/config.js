...
function resetSoulParticles() {
  if (!document.getElementById('soul')) return;
  const extras = document.getElementById('soul').querySelectorAll('.soul-extra-particle');
  extras.forEach(el => el.remove());
  extraSoulParticles = 0;
}

function addSoulParticle() {
  // ambient: částice kolem duše
  if (
  ...
  document.getElementById('soul').appendChild(dot);
  extraSoulParticles++;
}
...
function endLevel(reason = 'Level je u konce.') {
  ...
  // zapsat shrnutí pro případné uložení vizuálu
  lastRunSummary = {
    ...
    completedAll: (hits >= totalScheduled && totalScheduled > 0 && !gameOverBySoul)
  };

  // 100 % completion → odemknout další level (pokud existuje)
  if (lastRunSummary.completedAll) {
    unlockNextLevel(currentLevelKey);
  }

  // přičtení měny: měna = skóre
  if (score > 0) {
    addCurrency(score);
  }
  ...
  // info o měně ve výsledkovém overlayi
  if (currencyInfoEl) {
    if (score > 0) {
      currencyInfoEl.textContent = `Za tento run získáváš ${score} ✦ měny.`;
    } else {
      currencyInfoEl.textContent = 'Tentokrát jsi nezískal žádnou měnu.';
    }
  }
  ...
  if (saveVisualBtn) {
    ...
    if (playerState && playerState.savedVisual) {
      saveVisualBtn.textContent = `Přepsat uložený vizuál (cena ${VISUAL_SAVE_PRICE} ✦)`;
    } else {
  ...
  // po skončení levelu můžeme refreshnout levely v menu (kvůli odemykání)
  if (typeof refreshLevelsUI === 'function') {
    refreshLevelsUI();
  }
}
